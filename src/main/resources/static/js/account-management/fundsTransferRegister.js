$(document).ready(function() {
	...
	$("#showBtn").click(function() {
		...
		$("#showBtn").prop("disabled", true).text("Loading...");
		...
		$.ajax({
			...
			success: function(response) {
				...
				$("#showBtn").prop("disabled", false).text("SHOW DETAILS");
			},
		...
		data.forEach((item, index) => {
			...
			rows += `
                <tr>
                    ...
                    <td class="text-end">${closingBalance.toFixed(2)}</td>
                </tr>
            `;
		});
	}
...
function exportToExcel() {
	...
	XLSX.utils.book_append_sheet(wb, ws, "Fund Transfer");

	XLSX.writeFile(wb, "Fund_Transfer_Report.xlsx");
}
